---
layout: post
title: Blog Reboot
date: 2025-03-14 21:30:00
description: Why shouldn't I write a blog?
---

Over the years I've had a variety of outlets for my thoughts and projects. Social
media has been a primary outlet for casual authorship; and during my 2014 Google
Summer of Code internship with Biopython, I was required to maintain a blog
documenting my progress. I enjoyed the process of writing a blog but ultimately
my blog languished after the internship wrapped up.

When I started a first professional role at Google FiberI effectively stopped
writing in my leisure time; most of my creative and technical energy went into
that jobs. As my family grew, my time for side-projects shrank commensurately.

It's 2025 now and circumstances have shifted enough that I'm ready, and
motivated, to restart the habit. One big factor here is my decision to move away
from social media platforms. These sites used to be a significant outlet for my
writing and now that I don't have this outlet, a blog feels like a productive
way to scratch that itch. Linked-in (one of my few intact social media accounts)
would like to be that outlet, but I can't take that platform seriously nor do I
trust them with exclusive access to my data.

I've based my blog on the awesome [al-folio](https://github.com/alshedivat/al-folio)
theme for [Jekyll](https://jekyllrb.com/). This theme has more bells and
whistles than I can reasonably count. Over the next few months I'll be cleaning
and customizing this blog to suit my needs. At some point I'll migrate my old
BioPython posts here since changes to BlogSpot have purged them from the internet.
